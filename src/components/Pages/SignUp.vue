      ...
      <div class="row no-gutters text-center align-items-center justify-content-center min-vh-100">
        <div class="col-12 col-md-6 col-lg-5 col-xl-4">
          ...
          <p>
            ...
            <router-link to="/sign-in" title="Sign In" class="font-size-sm">
              Sign in </router-link
            >.
          </p>
        </div>
...
export default defineComponent({
  ...
  setup: () => {
    ...
    function doSignUp () {
      console.log('do signup code will goes here');
      console.log(signupform);
    }
    function update (payload: inputValue): void {
      if (payload.name == 'email') {
        signupform.email = payload.value;
      } else if (payload.name == 'password') {
        signupform.password = payload.value;
      } else if (payload.name == 'name') {
        signupform.name = payload.value;
      }
    }
    return { doSignUp, update, signupform };
  }
});
</script>
