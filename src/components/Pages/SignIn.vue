      ...
      <div class="row no-gutters text-center align-items-center justify-content-center min-vh-100">
        <div class="col-12 col-md-6 col-lg-5 col-xl-4">
            ...
            <div class="form-group d-flex justify-content-between">
              <div class="custom-control custom-checkbox">
                <input
                  ...
                  v-model="rememberMe"
                />
                <label class="custom-control-label text-muted font-size-sm" for="checkbox-remember"
                  >Remember me</label
                >
              </div>

              <router-link to="/reset-password" title="Reset password" class="font-size-sm">
                Reset password
              </router-link>
            </div>
            <button
          ...
          <p>
            ...
            <router-link to="/sign-up" title="Sign Up" class="font-size-sm">
              Sign up </router-link
            >.
          </p>
        </div>
...
import { inputValue } from '../../Config/Config';
...
export default defineComponent({
  ...
  setup: () => {
    ...
    function doLogin () {
      console.log('do login code will goes here');
      console.log(loginform);
    }
    function update (payload: inputValue): void {
      if (payload.name == 'email') {
        loginform.email = payload.value;
      } else if (payload.name == 'password') {
        loginform.password = payload.value;
      }
    }
    return { rememberMe, loginform, doLogin, update };
  }
});
</script>
