  ...
  <Modal :showModal="showCreateGroup" modalClass="py-0">
    <template v-slot:modalHeader>
      ...
      <button
        ...
        data-dismiss="modal"
        aria-label="Close"
        @click="toggleModal('create-group')"
      >
        <span aria-hidden="true">&times;</span>
      </button>
    </template>
    <template v-slot:modalBody>
      <CreateGroup
        :cuStep="createGrupCuStep"
        :creategroupobj="creategroupobj"
        @onFilePicked="groupPictureUpload"
      ></CreateGroup>
    </template>
    <template v-slot:modalFooter>
      <button
...
export default defineComponent({
  ...
  setup: () => {
    ...
    const dropdownCss = ref(
      'position: absolute; transform: translate3d(-128px, 40px, 0px); top: 0px; left: 0px; will-change: transform;'
    );
    const isRTLCn = ref(localStorage.getItem('rtl') ? localStorage.getItem('rtl') : isRTL);

    if (isRTLCn.value) {
      dropdownCss.value =
        'position: absolute; transform: translate3d(-8px, 40px, 0px); top: 0px; left: 0px; will-change: transform;';
    }

    function getNotificationsList () {
      Notifications.getAll()
        .then((response: ResponseType) => {
          notificationList.value = <notificationType[]>response.data;
        })
        .catch((e: Error) => {
          console.log(e);
        });
    ...
    function filteredFriendList (): friendType[] {
      let friendListAr = friendsList.value;
      if (friendListAr == undefined) return [];
      if (friendSearch.value === '' || friendSearch.value == null) return friendListAr;
    ...
    function createGrup (): void {
      ...
      console.log(
        creategroupobj.grpname,
        creategroupobj.friends,
        creategroupobj.type,
        creategroupobj.image
      );
