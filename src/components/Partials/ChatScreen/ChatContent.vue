  ...
  <div class="chat-content p-2" id="messageBody">
    <div class="container" id="msg-container">
      ...
      <div class="message-day" v-for="(converse, y) in filteredConversationList">
        <div class="message-divider sticky-top pb-2" :data-label="timeAgo(converse.date, false)">
          &nbsp;
        </div>

        <div
          class="message"
          :class="[{ self: conversemsg.is_sent }]"
          v-for="(conversemsg, i) in converse.list"
        >
          <div class="message-wrapper">
            <div class="message-content">
              <h6 class="text-dark" v-if="conversation.type == 'group'">{{ conversemsg.name }}</h6>
              <span v-if="conversemsg.message_type.includes('text')">{{
                conversemsg.message
              }}</span>
              ...
              <div class="document" v-if="conversemsg.message_type == 'document'">
                ...
                <div class="document-body">
                  <h6>
                    <a
                      :href="conversemsg.message_link"
                      class="text-reset"
                      :title="conversemsg.message"
                      >{{ conversemsg.message }}</a
                    >
                  </h6>
              ...
              <h6 v-if="conversemsg.message_type == 'image' && conversemsg.message != ''">
                {{ conversemsg.message }}
              </h6>
              <div
                class="form-row"
                v-if="conversemsg.message_type == 'image' && conversemsg.message_link.length"
              >
                <div
                  class="col"
                  @click="showImg(index)"
                  v-for="(img, index) in conversemsg.message_link"
                >
          ...
          <div class="message-options">
            ...
            <DropdownWithSlot
              ...
              dpstyle="position: absolute; transform: translate3d(0px, -121px, 0px); top: 0px; left: 0px; will-change: transform;"
            >
              <template v-slot:menus>
                <a class="dropdown-item d-flex align-items-center" href="#">
                  <DuplicateIcon
                    class="injectable hw-18 mr-2"
                ...
                <a class="dropdown-item d-flex align-items-center" href="#">
                  <ReplyIcon
                    class="injectable rotate-y hw-18 mr-2"
                    alt="message forward"
                  ></ReplyIcon>
                  <span>Forward</span>
                </a>
