      ...
      <a class="contacts-link" href="javascript:void(0)" @click="openCall(call)">
        ...
        <div class="contacts-content">
          <div class="contacts-info">
            <h6 class="chat-name text-truncate">{{ call.user_name }}</h6>
          </div>
          <div class="contacts-texts">
            <PhoneIncoming
              class="injectable hw-16 text-muted mr-1"
              v-if="call.type == 'incoming'"
            ></PhoneIncoming>
            <PhoneOutgoing
              class="injectable hw-16 text-muted mr-1"
              v-if="call.type == 'outgoing'"
            ></PhoneOutgoing>
            <PhoneIncoming
              class="injectable hw-16 text-danger mr-1"
              v-if="call.type == 'missed-calls'"
            ></PhoneIncoming>
            <p class=" mb-0" :class="call.type == 'missed-calls' ? 'text-danger' : 'text-muted'">
              20 mins ago
            </p>
          </div>
        </div>
...
import { sgconvdetail } from '../../Config/Config';
export default defineComponent({
  ...
  setup: () => {
    ...
    function openCall (conversation: sgconvdetail): void {
